...
from src.framework.ppg.Graph import Node, Edge, Graph
from src.framework.ppg.database import database

if tp.TYPE_CHECKING:
    from src.framework.ppg.Graph import SubNode, SubEdge, SubGraph
...
class GraphParser(object):
    ...
    def save(
        ...
        for edge in graph.get_edges():
            ...
            ids: str = ' '.join([f'{node.id()}' for node in edge.get_nodes()])
        ...
        for i, line in enumerate(lines):
            ...
            if tag == 'FIX':
                ...
                if issubclass(element_type, Node):
                    ...
                    # read node
                    id_: int = int(words[0])
                    node: 'SubNode' = element_type(None, id_)
                    words = words[1:]
