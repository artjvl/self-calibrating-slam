...
Key = tp.Tuple[tp.Type[Supported], tp.Tuple[tp.Type[SubFactorNode], ...]]
...
class EdgeFactory(object):
    ...
    def from_types_measurement_nodes(
            ...
            *node_types: tp.Type[SubFactorNode]
    ) -> tp.Type[SubCalibratingEdge]:
    ...
    def from_measurement_nodes(
        ...
        node_types: tp.Tuple[tp.Type[SubFactorNode], ...] = tuple(type(node) for node in nodes)
